...
public class saveGameFile : MonoBehaviour
{
    ...
    public void startSaveData()
    {
        ...
        if (UsernameInput.userCountAdd || activeUser != UsernameInput.newusername)
        {
            activeUser = UsernameInput.newusername;
            File.AppendAllText(txtDocumentName, uN + uCount + "[" + curUn + "]" + pT + uCount + "[" + curPt + "]end" + uCount + "\n");
            uCount++;

            UsernameInput.userCountAdd = false;
        }
    ...
    public void printToHighScores()
    {
        ...
        for (int i = 0; i < uN_list.Capacity; i++)
        {
            ...
            totalScore.text = totalScore.text + pT_list[i] + " /" + maxPoints + "\n";
